...
import { isHealthy, isSafe } from "./lib/ranking.mjs";
...
// health_score가 너무 낮은(archived되었거나 라이선스도 없고 아무도 fork 안 한) 것,
// audit_score가 너무 낮은(Socket/Snyk 등에서 위험 신호가 나온) 것도 제외한다.
const installable = catalog.filter(
  (it) => it.type === "skill" && it.install && !it.weak_match && isHealthy(it) && isSafe(it)
);
...
for (const [key, items] of groups.entries()) {
  ...
  picks.push({
    ...
    audit_score: top.audit_score ?? null,
    install: top.install
  });
}
  ...
  JSON.stringify(
    {
      ...
      granularity: per,
      note: "installs(설치 수) 기준 인기 1위이지, 실측 성능/품질 순위가 아님. archived·라이선스 없음·fork 없음(health_score)이거나 Socket/Snyk 등에서 위험 신호가 나온(audit_score) 항목은 미리 제외함 — ARCHITECTURE.md/BENCHMARK.md의 한계 참고",
      picks
    },
    null,
    2
