...
const installable = catalog.filter((it) => it.type === "skill" && it.install && !it.weak_match);
  ...
  JSON.stringify(
    {
      ...
      granularity: per,
      note: "installs(설치 수) 기준 인기 1위이지, 실측 성능/품질 순위가 아님 — ARCHITECTURE.md/BENCHMARK.md의 한계 참고",
      picks
    },
    null,
    2
