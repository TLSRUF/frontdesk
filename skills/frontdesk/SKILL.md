...
2. 스크립트가 2단계(단일 카테고리) 또는 3단계(다중 카테고리)로 후보를 반환하면, 그 카테고리 이름과 후보 목록을 **`--install` 없이** 먼저 사용자에게 보여주고 어떤 걸 쓸지 확인한다.

3. 사용자가 승인하면 **같은 명령을 `--install`만 붙여서 다시 실행한다** — 캐시에 남아있어 재매칭 없이 그 자리에서 바로 설치까지 된다:

   ```bash
   node scripts/route.mjs "<같은 작업 설명>" --install
   ```

   `type: "skill"`인 후보(skills.sh 스킬, `owner/repo@skill` 형태)만 `npx skills add`로 자동 설치되고, 순수 GitHub 저장소 후보는 안전을 위해 자동 설치하지 않고 `git clone` 명령만 안내한다. **`--install` 없이 먼저 확인받는 순서를 절대 건너뛰지 말 것** — skills CLI 자신도 "이 스킬들은 full agent permission으로 돌아간다"고 경고한다.

4. 스크립트가 **4단계**(매칭 0건)를 반환해도 곧바로 "카탈로그에 없다"고 단정하지 말 것. 이 스크립트는 영어 키워드만 매칭하는 규칙 기반이라 다음과 같은 경우 false negative가 흔하다:
...
5. 최종 추천은 **항상 가장 좁은 것부터** 제시한다. 카탈로그에 있는 스킬 하나로 되는 일에 멀티에이전트 스웜이나 풀스택 서브에이전트를 부르는 건, ponytail이 경계하는 "date picker에 flatpickr 설치하기"와 같은 과잉설계다.

## 스타터팩: 부서별 대표 스킬 한 번에 설치

"이거 하나만 설치하면 부서별로 제일 나은 스킬들이 알아서 다 갖춰졌으면 좋겠다"는 요청에는 이 기능을 쓴다. `data/starter-pack.json`에 13개 부서 중 신뢰도 높게 분류된(= weak_match 아닌), 방치되지 않고 안전한(`health_score`/`audit_score` 통과) `type: skill` 항목 중 **설치 수(installs) 1위**를 부서마다 하나씩 골라뒀다 — 13개 부서 전부 픽이 있다.
...
- 순위는 여전히 `stars`/`installs`(인기도) 기준이다 — `health_score`(archived·라이선스·fork 여부)와 `audit_score`(Socket/Snyk 등 보안 감사)로 방치되었거나 위험한 저장소는 걸러내지만, 그 필터를 통과한 후보들 사이의 순서는 인기 ≠ 성능이라는 한계가 그대로 있다
