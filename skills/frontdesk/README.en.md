...
Or install directly from the repo via Claude Code (recommended until this is listed on the skills.sh index):
...
- `scripts/` — the collect (`collect-*.mjs`) → classify (`classify.mjs`) → enrich (`enrich-descriptions.mjs`) → document (`build-catalog-md.mjs`) → route (`route.mjs`) → benchmark (`benchmark.mjs`) pipeline
...
Refresh the catalog with `npm run all`, run the benchmark with `npm run benchmark` (see the repo root README).
