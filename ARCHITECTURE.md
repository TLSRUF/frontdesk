...
이 문서는 [`CATALOG.md`](CATALOG.md)에 정리된 1,311개의 에이전트/스킬 데이터를 기반으로, "아이디어 → 수익모델 → 마케팅 → 디자인 → 개발"을 한 번에 처리하는 통합 AI 툴을 어떻게 만들지에 대한 설계 방향을 정리한다. Phase 2에서 판단 사다리의 규칙 기반 프로토타입(`scripts/route.mjs`)까지 구현했다 — 아래 "라우터 프로토타입" 섹션 참고.
...
```bash
node scripts/route.mjs "새 기능 랜딩페이지 SEO 최적화하고 이메일 뉴스레터도 보내야 해"
# → 2단계, "9.1 마케팅/그로스 > SEO" 카테고리, 상위 후보 3개 제시
```

동작:
- **1단계(캐시)**: `data/route-cache.json`에 정규화된 요청 텍스트를 키로 저장 — 동일 요청 재입력 시 아무 매칭도 다시 하지 않고 즉시 반환
...
- **라우팅**: `scripts/route.mjs` — 판단 사다리 프로토타입 (위 섹션 참고)
